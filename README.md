...
Sam’s Voice now plays through a page-scoped Web Audio context, shared with the microphone analyser. Tracks are stopped after each answer, while the unlocked context is reused. This avoids HTML media playback between recognition sessions, a trigger described in [WebKit issue 321436](https://bugs.webkit.org/show_bug.cgi?id=321436). That report matches the symptom; it does not establish the cause on an untested phone.

The microphone shows connecting, listening and reconnecting states plus an input-level meter. Recognition has startup and no-result watchdogs, two bounded recovery attempts, interrupted-context and track checks, and a final-result deadline. When recovery fails, the draft stays editable and the user can tap Speak again. Add `?dev=true`, then open **Lesson workshop → Microphone diagnostics** to see the last 30 state/error events. These events stay in memory and contain no audio or recognized words. Automated diagnostics cover 12 alternating teacher-playback/spoken-answer turns, stalled recognition, interrupted audio, permission delays, cancellation and missing final events. Actual phone/browser recognition still needs a device check.
